...
Sint16 *g_worldMap_Layer_1[8];

void setup_cmd_parser(Sint16 *pWordMap)
{
  g_worldMap_Layer_1[0] = pWordMap;
}
...
void parseCmd(char *_szCmd)
{
  ...
  else if (strcmp(szTokens[0], "save") == 0)
  {
    ...
    // SDL_RWwrite(rw, g_worldMap_Layer_1[0], sizeof(Uint16), 64);
    // SDL_RWclose(rw);
  ...
  else if (strcmp(szTokens[0], "load") == 0)
  {
    ...
    SDL_RWread(rw, g_worldMap_Layer_1[0], sizeof(Uint16), 64);
    SDL_RWclose(rw);
  }
  else if (strcmp(szTokens[0], "new") == 0)
  {
    memset(g_worldMap_Layer_1[0], -1, 128);
  }
