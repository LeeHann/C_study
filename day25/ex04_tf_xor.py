...
BATCH_SIZE = 1  # 데이터 단위
train_dataset = train_dataset.shuffle(4).batch(BATCH_SIZE)  # batch : 데이터 반복 주기
...
def create_model():
    ...
    sgd = tf.keras.optimizers.SGD(
        lr=0.01, decay=0, momentum=0.99, nesterov=True)
...
if __name__ == "__main__":
    ...
    model.fit(train_dataset, epochs=500,
              validation_data=test_dataset)  # epochs : 반복 학습 횟수
