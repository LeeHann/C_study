...
Uint16 g_worldMap_Layer1[64];

int main(int argc, char *argv[])
{
  for(int i=0; i<64; i++){
    g_worldMap_Layer1[i] = -1;
  }
  ...
  while (bLoop)
  {
    ...
    for(int i=0; i<64; i++){
      if(g_worldMap_Layer1[i] != -1){
        putTile(g_pRenderer, g_pTitleTexture, i%8, i/8, g_worldMap_Layer1[i]);
      }
    }
